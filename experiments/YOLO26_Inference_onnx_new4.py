...
def _is_nhwc(shape): return len(shape) == 4 and shape[-1] == 3
def _is_nchw(shape): return len(shape) == 4 and shape[1] == 3

def main():
    ap = argparse.ArgumentParser()
    ap.add_argument('--model-path', type=str, default='yolo26n_e2e.onnx')
    ...
    if _is_nhwc(ishape):
        layout = "NHWC"; m_h = int(ishape[1] or 640); m_w = int(ishape[2] or 640)
    elif _is_nchw(ishape):
        layout = "NCHW"; m_h = int(ishape[2] or 640); m_w = int(ishape[3] or 640)
    else:
        layout = "NCHW"; m_h = 640; m_w = 640

    # preprocess (float32 0-1)
    t0 = time()
    ...
    input_tensor = rgb[None, ...] if layout == "NHWC" else np.transpose(rgb, (2, 0, 1))[None, ...]
